...
function drawDomains() {
  Object.entries(domains).forEach(([id, zone]) => {
    const layout = domainGeometry(id); const { x, y, radiusX, radiusY } = layout; const mastery = domainProgress(id); ctx.save(); ctx.globalAlpha = .12; ctx.fillStyle = zone.color; ctx.beginPath(); ctx.ellipse(x, y, radiusX, radiusY, 0, 0, Math.PI * 2); ctx.fill(); ctx.globalAlpha = 1; ctx.strokeStyle = `${zone.color}55`; ctx.setLineDash([3, 5]); ctx.lineWidth = 1; ctx.beginPath(); ctx.ellipse(x, y, radiusX, radiusY, 0, 0, Math.PI * 2); ctx.stroke(); ctx.setLineDash([]); ctx.strokeStyle = '#2e9b80'; ctx.lineWidth = 2; ctx.beginPath(); ctx.ellipse(x, y, radiusX - 5, radiusY - 5, 0, -Math.PI / 2, -Math.PI / 2 + Math.PI * 2 * mastery / 100); ctx.stroke(); ctx.fillStyle = zone.color; ctx.globalAlpha = .76; ctx.font = '600 10px Inter, Microsoft YaHei, sans-serif'; ctx.textAlign = 'center'; ctx.fillText(`${zone.short} · ${mastery}%`, x, Math.max(14, y - radiusY + 12)); ctx.restore();
  });
}
...
<template>
  <section class="graph-page">
    <div class="graph-head"><div><p class="eyebrow">ARCHITECT CAPABILITY ATLAS</p><h2>架构师能力星图</h2><p>以知识树为坐标：节点越大，越值得优先投入时间</p><div class="atlas-progress"><b>{{ overallMastery }}%</b><span>总体掌握</span><i><em :style="{ width: `${overallMastery}%` }"></em></i><span>{{ masteredCount }}/{{ nodeTotal }} 已掌握</span></div></div><BaseButton size="sm" variant="ghost" icon="rotate-ccw" @click="resetView">重置视图</BaseButton></div>

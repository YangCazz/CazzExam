...
function render() {
  ...
  [...graph.nodes].sort((a, b) => a.dR - b.dR).forEach(n => { const matched = !query || n.name.includes(query); const active = !focus || focus.has(n); const radius = n.dR; const color = rgb(nodeColor(n)); ctx.save(); ctx.globalAlpha = n.dA * (matched && active ? 1 : (query ? .1 : .16)); const fill = ctx.createRadialGradient(n.x - radius * .28, n.y - radius * .28, 0, n.x, n.y, radius); fill.addColorStop(0, `rgb(${Math.min(255, color.r + 34)},${Math.min(255, color.g + 34)},${Math.min(255, color.b + 34)})`); fill.addColorStop(1, nodeColor(n)); ctx.fillStyle = fill; ctx.beginPath(); ctx.arc(n.x, n.y, radius, 0, Math.PI * 2); ctx.fill(); ctx.strokeStyle = info.value === n ? '#f3c96b' : hover.value === n ? '#fff' : 'rgba(255,255,255,.7)'; ctx.lineWidth = info.value === n || hover.value === n ? 2.6 : 1.4; ctx.stroke(); if (radius >= 16) { ctx.fillStyle = '#fff'; ctx.shadowColor = 'rgba(0,0,0,.24)'; ctx.shadowBlur = 1; ctx.font = `600 ${Math.max(9, Math.min(12, radius * .46))}px Inter, Microsoft YaHei, sans-serif`; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; const visible = n.name.length > 5 ? `${n.name.slice(0, 4)}…` : n.name; ctx.fillText(visible, n.x, n.y); ctx.shadowColor = 'transparent'; } ctx.restore(); });
...
<template>
  <section class="graph-page">
    <div class="graph-head"><div><p class="eyebrow">KNOWLEDGE RELATION MAP</p><h2>知识地图网络</h2><p>拖拽节点探索 · 右键固定详情 · 搜索高亮跳转</p></div><BaseButton size="sm" variant="ghost" icon="rotate-ccw" @click="resetView">重置视图</BaseButton></div>

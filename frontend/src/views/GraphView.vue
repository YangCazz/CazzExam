...
.detail-card{max-height:min(46vh,340px);overflow-y:auto;margin-top:12px}
.atlas-top{display:grid;grid-template-columns:220px minmax(0,1fr);gap:18px;align-items:stretch}
.progress-card{display:flex;flex-direction:column;justify-content:center;min-height:126px;padding:16px 18px;border:1px solid var(--border);border-radius:var(--radius);background:var(--surface-solid)}
.progress-card p{margin:0 0 5px;color:var(--text-faint);font:700 10px var(--mono);letter-spacing:.06em}
.progress-card strong{color:var(--action-primary);font:700 30px/1 var(--mono);letter-spacing:-.05em}
.progress-card>span{color:var(--text-muted);font-size:11px}
.progress-card>div{display:flex;align-items:center;gap:8px;margin-top:12px}
.progress-card i{display:block;flex:1;height:6px;overflow:hidden;border-radius:6px;background:var(--surface-inset)}
.progress-card em{display:block;height:100%;border-radius:inherit;background:var(--action-primary)}
.progress-card b{color:var(--text-muted);font:600 10px var(--mono);white-space:nowrap}
.progress-card small{margin-top:10px;color:var(--text-faint);font-size:10px}
.atlas-main{display:grid;gap:10px;min-width:0;padding:4px 0}
.atlas-main .graph-head{align-items:center}
.atlas-main .map-search{width:min(390px,40%)}
.atlas-main .map-count{margin-left:auto}
@media(max-width:820px){.atlas-top{grid-template-columns:1fr}.progress-card{min-height:0}.atlas-main .graph-head{align-items:flex-start;flex-direction:column}.atlas-main .map-search{width:min(400px,100%)}.atlas-main .map-count{margin-left:0}}
</style>

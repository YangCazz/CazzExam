...
<template>
  <div class="card"><h2>目标与考试</h2><p class="muted">设置仅保存在本机，用于生成学习节奏；未设置考试日期也可以正常学习。</p><div class="settings-grid"><label>备考资格<input v-model="profile.certification" /></label><label>目标考试日期<input type="date" v-model="profile.target_date" /></label><label>每周可用时间（分钟）<input type="number" min="30" max="2400" v-model.number="profile.weekly_minutes" /></label></div><BaseButton @click="save">保存设置</BaseButton><p v-if="msg" class="badge ok">{{ msg }}</p><p v-if="err" class="badge err">{{ err }}</p></div>
  <div class="card"><h2>本地数据与备份</h2><p class="muted">学习记录保存在 <code>backend/data/study.db</code>。恢复备份会覆盖当前数据，请先另行备份。</p><div class="row"><a class="btn ghost" href="/api/questions/export" download="questions.json">导出题库 JSON</a><a class="btn ghost" href="/api/knowledge/graph" download="knowledge-graph.json">导出知识图谱</a></div></div>

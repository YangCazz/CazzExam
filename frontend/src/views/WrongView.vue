...
<template>
  ...
  <section v-if="current" class="review-layout"><article class="panel recall-card"><div class="recall-top"><span class="type-pill amber">待回忆</span><span>第 {{ active + 1 }} / {{ queue.length }} 题</span></div><p class="question-stem">{{ current.question?.stem }}</p><div v-if="current.question?.qtype === 'choice'" class="recall-options"><span v-for="opt in current.question.options" :key="opt">{{ opt }}</span></div><textarea v-model="reflection" rows="4" placeholder="不要急着看答案：先写下你认为的答案或解题依据。"></textarea><div v-if="!revealed" class="row end"><button @click="revealed = true">我已完成回忆，查看反馈 →</button></div><div v-else class="answer-reveal"><p>参考答案 <b>{{ current.question?.answer }}</b></p><p class="muted">{{ current.question?.analysis || '此题暂无解析，请记录自己的正确思路。' }}</p></div></article>
    <aside class="panel review-decision"><p class="eyebrow">DECISION</p><h3>这次回忆的质量？</h3><p>评价的是“无需提示能否提取”，不是做题时的感觉。</p><label>主要错因<select v-model="errorType"><option>知识性错误</option><option>理解偏差</option><option>审题失误</option><option>方法错误</option><option>其他</option></select></label><button :disabled="!revealed" class="ghost" @click="grade(0)">没想起来 · 明天再见</button><button :disabled="!revealed" class="ghost" @click="grade(3)">模糊但可恢复 · 稍后复习</button><button :disabled="!revealed" @click="grade(5)">清楚回忆 · 拉长间隔</button></aside></section>
  <section v-else class="panel empty-review"><p class="eyebrow">INBOX ZERO</p><h2>当前没有到期复习。</h2><p>下一次练习中答错的题目会自动进入此处，按间隔重复安排。</p><router-link class="btn" to="/practice">去做一段针对训练</router-link></section>
